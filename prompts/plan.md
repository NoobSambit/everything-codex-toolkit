...
Run inline by default. Do not call a subagent by default. This keeps the planning workflow usable in plain Codex sessions.
...
```
User: Use prompts/plan.md to plan real-time notifications when markets resolve
...
- Use `prompts/code-review.md` to review completed implementation
...
Everything Codex Toolkit also provides a `planner` role prompt for installs that include agent files. Use it only when the local runtime exposes that role and the user explicitly asks you to delegate planning.
